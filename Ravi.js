...
const GuruDetailsScreen = ({ route }) => {
    ...
    useEffect(() => {
        // Initially set the title to empty or a placeholder
        navigation.setOptions({ title: '' });

        const fetchGuruDetails = async () => {
            try {
                const response = await fetch(`https://my.bmusician.com/app/gurudetails/2d0701d5-c41c-404b-97bf-e3b6ed3595fd`);
                const json = await response.json();
                if (json.success) {
                    setGuruDetails(json.data);
                    // After fetching, set the actual title to the Guru's name
