...
import defaultImage from '../../Assets/Instruments/Keyboard.png'
...
const KeyGurus = () => {
    ...
    useEffect(() => {
        fetch('https://my.bmusician.com/app/GetGurus/?faculty=keys')
            .then(response => response.json())
            .then(data => {
                const keysGurus = data.gurus.filter(guru =>
                    guru.SpecializationNames.includes('keyboard')
                );
                setGurus(keysGurus);
            })
            .catch(error => {
