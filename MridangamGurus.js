...
import defaultImage from '../../Assets/Instruments/mridangam.png'
import { useNavigation } from '@react-navigation/native';

const MridangamGurus = () => {
    const navigation = useNavigation();
    const [gurus, setGurus] = useState([]);

    useEffect(() => {
        fetch('https://my.bmusician.com/app/GetGurus/?faculty=percussion')
            .then(response => response.json())
            .then(data => {
                const mridangamGurus = data.gurus.filter(guru =>
                    guru.SpecializationNames.includes('mridangam')
                );
                setGurus(mridangamGurus);
            })
            .catch(error => {
