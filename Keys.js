...
const KeysGuruList = () => {
    ...
    useEffect(() => {
        const fetchGurus = async () => {
            try {
                const response = await fetch('https://my.bmusician.com/app/GetGurus/?faculty=keys');
        ...
        <FlatList
            ...
            renderItem={({ item }) => (
                <TouchableOpacity onPress={() => GuruDetail(item.ID)} style={styles.guruContainer}>
                    ...
                    <Image
                        source={item.ProfilePicture ? { uri: `https://my.bmusician.com${item.ProfilePicture}` } : require('../Assets/Instruments/Keyboard.png')}
